...
#from leafnode import LeafNode # Maybe this import should not be here?
...
class HNode(metaclass=ABCMeta):
	...
	def _dump_tree(self, depth, leaf_count, buff):
		...
		Args:
			depth (int):
			leaf_count (int):
			buff (list[str]): A wrapper for a single string, making it mutable
		"""
		max_value = -1
		class_val = ''
		for class_value, mass in self.class_distribution.items():
	...
	def graph_tree(self, text):
		...
		for class_value, mass in self.class_distribution.items():
			if mass.weight > max_value:
				max_value = mass.weight
				class_val = class_value
		text[0] += 'N{0} [label=\"{1} ({2})\"]\n'.format(self._node_num, class_val, max_value)

	# Work out this string buffer thing
	def __str__(self, print_leaf=False):
		self.install_node_nums(0)
		# Wrapper for a string, making it mutable
		buff = ['']
